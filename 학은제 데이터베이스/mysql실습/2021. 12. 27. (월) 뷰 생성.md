# 2021. 12. 27. (월)

## mysql

### 뷰 생성

```

Microsoft Windows [Version 10.0.18362.356]
(c) 2019 Microsoft Corporation. All rights reserved.

C:\Users\user>mysql -u thkim -p'
mysql: [Warning] Using a password on the command line interface can be insecure.
ERROR 1045 (28000): Access denied for user 'thkim'@'localhost' (using password: YES)

C:\Users\user>mysql -u thkim -p
Enter password:
Welcome to the MySQL monitor.  Commands end with ; or \g.
Your MySQL connection id is 19
Server version: 8.0.27 MySQL Community Server - GPL

Copyright (c) 2000, 2021, Oracle and/or its affiliates.

Oracle is a registered trademark of Oracle Corporation and/or its
affiliates. Other names may be trademarks of their respective
owners.

Type 'help;' or '\h' for help. Type '\c' to clear the current input statement.

mysql> use itbank
Database changed
mysql> select * from customer;
+---------+--------+--------+--------+-------------+
| account | name   | grade  | credit | address     |
+---------+--------+--------+--------+-------------+
| apple   | 이남이 | VIP    |   5000 | 경기 용인시 |
| bank    | 홍길동 | Gold   |  30000 | 서울 종로구 |
| bird    | 구선두 |        |      0 | 충남 천안시 |
| eagle   | 박세째 | Gold   |   2450 |  부산 남구  |
| king    | 오나라 | Gold   |  15000 | NULL        |
| moon    | 한가위 | New    |   1000 | 서울 용산구 |
| pencil  | 김둘   | Silver |    350 |  경기 수원  |
| sun     | 박태양 | New    |   1000 | 용인 처인구 |
| table   | 정탁자 | New    |      0 | NULL        |
| watch   | 김정확 | New    |   NULL | NULL        |
+---------+--------+--------+--------+-------------+
10 rows in set (0.03 sec)

mysql> CREATE VIEW GoodCustomer ( account, name, credit )
    -> AS SELECT account, name, credit
    -> FROM customer
    -> WHERE grade='VIP' OR grade='Gold'
    -> WITH CHECK OPTION;
Query OK, 0 rows affected (0.06 sec)

mysql> SELECT * FROM GoodCustomer;
+---------+--------+--------+
| account | name   | credit |
+---------+--------+--------+
| apple   | 이남이 |   5000 |
| bank    | 홍길동 |  30000 |
| eagle   | 박세째 |   2450 |
| king    | 오나라 |  15000 |
+---------+--------+--------+
4 rows in set (0.01 sec)

mysql> DESC GoodCustomer;
+---------+-------------+------+-----+---------+-------+
| Field   | Type        | Null | Key | Default | Extra |
+---------+-------------+------+-----+---------+-------+
| account | varchar(10) | NO   |     | NULL    |       |
| name    | varchar(10) | NO   |     | NULL    |       |
| credit  | int         | YES  |     | 0       |       |
+---------+-------------+------+-----+---------+-------+
3 rows in set (0.02 sec)

mysql> CREATE VIEW GoodCustomer1
    -> AS SELECT account, name, credit
    -> FROM customer
    -> WHERE grade='VIP' OR grade='Gold'
    -> WITH CHECK OPTION;
Query OK, 0 rows affected (0.02 sec)

mysql> DESC GoodCustomer1;
+---------+-------------+------+-----+---------+-------+
| Field   | Type        | Null | Key | Default | Extra |
+---------+-------------+------+-----+---------+-------+
| account | varchar(10) | NO   |     | NULL    |       |
| name    | varchar(10) | NO   |     | NULL    |       |
| credit  | int         | YES  |     | 0       |       |
+---------+-------------+------+-----+---------+-------+
3 rows in set (0.00 sec)

mysql> CREATE VIEW GoodCustomerBigCredit
    -> AS SELECT account, credit
    -> FROM GoodCustomer
    -> WHERE credit > 10000
    -> WITH CHECK OPTION;
Query OK, 0 rows affected (0.02 sec)

mysql> DESC GoodCustomerBigCredit;
+---------+-------------+------+-----+---------+-------+
| Field   | Type        | Null | Key | Default | Extra |
+---------+-------------+------+-----+---------+-------+
| account | varchar(10) | NO   |     | NULL    |       |
| credit  | int         | YES  |     | 0       |       |
+---------+-------------+------+-----+---------+-------+
2 rows in set (0.00 sec)

mysql> SELECT * FROM GoodCustomerBigCredit;
+---------+--------+
| account | credit |
+---------+--------+
| bank    |  30000 |
| king    |  15000 |
+---------+--------+
2 rows in set (0.00 sec)

mysql> SELECT * FROM orders;
+-----+----------+------+-----+------------+-------------+
| num | customer | item | qty | date       | saddr       |
+-----+----------+------+-----+------------+-------------+
|   1 | apple    | E06  |  10 | 2020-01-01 | 경기 용인시 |
|   2 | apple    | E05  |   2 | 2020-01-01 | 경기 용인시 |
|   3 | eagle    | E01  |   3 | 2020-02-03 | 부산 남구   |
|   4 | bank     | E04  |   5 | 2020-02-03 | 서울 종로구 |
|   5 | bird     | E01  |  10 | 2020-03-04 | 충남 천안시 |
|   6 | bird     | E04  |   1 | 2020-03-04 | 충남 천안시 |
|   9 |          | E02  |   3 | 2020-04-01 | 제주시      |
+-----+----------+------+-----+------------+-------------+
7 rows in set (0.00 sec)

mysql> CREATE VIEW OrdersByCustomer ( customername, no_of_orders )
    -> AS SELECT customer, COUNT(*)
    -> FROM orders
    -> GROUP BY customer;
Query OK, 0 rows affected (0.01 sec)

mysql> SELECT * FROM OrdersByCustomer;
+--------------+--------------+
| customername | no_of_orders |
+--------------+--------------+
| apple        |            2 |
| eagle        |            1 |
| bank         |            1 |
| bird         |            2 |
|              |            1 |
+--------------+--------------+
5 rows in set (0.01 sec)

mysql> DESC OrdersByCustomer;
+--------------+-------------+------+-----+---------+-------+
| Field        | Type        | Null | Key | Default | Extra |
+--------------+-------------+------+-----+---------+-------+
| customername | varchar(10) | NO   |     | NULL    |       |
| no_of_orders | bigint      | NO   |     | 0       |       |
+--------------+-------------+------+-----+---------+-------+
2 rows in set (0.00 sec)

mysql> quit
Bye

```